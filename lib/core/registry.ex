defmodule TelemetryMetricsPrometheus.Core.Registry do
  ...
  def init(opts) do
    ...
    aggregates_plot_table_id = create_table(String.to_atom("#{name}_plot"), :set)
    ...
    state = %{
      config: %{aggregates_table_id: aggregates_table_id, dist_table_id: dist_table_id, aggregates_plot_table_id: aggregates_plot_table_id},
      metrics: [],
      do_backup: 0
    }
  ...
  def terminate(_reason, %{metrics: metrics, config: config} = _state) do
    with :ok <- Enum.each(metrics, &unregister_metric/1),
         ...
         true <- :ets.delete(config.aggregates_plot_table_id),
